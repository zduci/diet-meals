...
describe MealClassifier do
  ...
  let(:allows_meat) { stub(:exclusive => true, :allowed_foods => [meat], :forbidden_foods => []) }
  let(:empty_exclusive_diet) { stub(:exclusive => true, :allowed_foods => [], :forbidden_foods => []) }
  ...
  it 'accepts meals containing allowed ingredients' do
    allows_chicken = stub(:exclusive => true, :allowed_foods => [chicken], :forbidden_foods => [])
  ...
  it 'rejects meals containing forbidden ingredients with accepted child foods' do
    allows_meat_except_chicken = stub(:exclusive => true, :forbidden_foods => [chicken], :allowed_foods => [meat])
  ...
  it 'rejects meals containing forbidden ingredients with accepted descendant foods' do
    allows_meat_except_chicken_breast = stub(:exclusive => true, :forbidden_foods => [chicken_breast], :allowed_foods => [meat])
